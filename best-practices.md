...
function parseBetterJSAlternative(code) {
  ...
  var tokens = [];
  REGEXES.forEach(function mapRegex(REGEX) {
    statements.forEach(function mapStatement(statement) {
      // ...
    });
  });

  var ast = [];
  tokens.forEach(function mapToken(token) {
    // lex...
  });

  ast.forEach(function parse(node) {
    // parse...
  });
}
```
